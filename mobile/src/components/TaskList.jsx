...
function TaskList(props) {
    ...
    const handleUpdateCategory = async (updated) => {
        ...
        try {
            const response = await fetch(`https://todolist-rhhd.onrender.com/categories/${updated.id}`, {
                method: 'PUT',
                headers: {
    ...
    const handleDelete = async () => {
        const token = localStorage.getItem('token');
        try {
            const response = await fetch(`https://todolist-rhhd.onrender.com/categories/${props.id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`,
                },
