...
function ModalTask({ task, onClose, tasktypes, triggerRefresh }) {
    ...
    const handleSubmit = async () => {
        ...
        const url = task.id
            ? `https://todolist-rhhd.onrender.com/tasks/${task.id}`
            : 'https://todolist-rhhd.onrender.com/tasks';
    ...
    const handleDelete = async () => {
        const token = localStorage.getItem('token');
        try {
            const response = await fetch(`https://todolist-rhhd.onrender.com/tasks/${task.id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`,
                },
